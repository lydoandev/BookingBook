...
export default class Detail extends Component {
  ...
  getRelatedBook = async () => {
    var data = await callAPI(`api/books/${this.props.item.Id}/relatedBooks`, 'GET');
    this.setState({
      loading: false,
  ...
  render() {
    var { Authors, Title, OverallStarRating, Price, Quantity, TotalReview, Content, Medias, Id } = this.props.item;
    ...
    return (
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.info}>
          <Image source={{ uri: Medias[0].ImageAppUrl }}
        ...
        <View style={{ marginVertical: 30 }}>
          <Text style={styles.textContent} numberOfLines={6}>{Content}</Text>
        </View>
        <TitleSection type='Sách tương tự'></TitleSection>
        <ListBook data={relatedBooks} navigateToDetail={this.navigateToDetail}></ListBook>
        <Text style={{ fontSize: 20 }}>Nhận xét</Text>
        <View style={{ marginVertical: 30, justifyContent: 'center', alignItems: 'center' }}>
          <TouchableOpacity style={{ borderWidth: 1, borderRadius: 5, height: 50, justifyContent: 'center', alignItems: 'center', padding: 10 }}>
